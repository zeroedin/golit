		...
		Plugins: []api.Plugin{
			...
			// import styles from './foo.css' with { type: 'css' }
			{
				...
				Setup: func(build api.PluginBuild) {
					// Intercept .css imports and convert them to CSSStyleSheet-like modules
					build.OnResolve(api.OnResolveOptions{Filter: `\.css$`},
						func(args api.OnResolveArgs) (api.OnResolveResult, error) {
							// Only intercept if it's an import with assertion (detected by
							// resolving relative to source dir)
							resolved := args.Path
					...
					build.OnLoad(api.OnLoadOptions{Filter: ".*", Namespace: "css-module"},
						func(args api.OnLoadArgs) (api.OnLoadResult, error) {
							...
							// Export as a CSSResult-like object that Lit understands
			...
			// Plugin 2: Stub only universal Lit SSR packages (we provide our own shim)
			{
				Name: "ssr-stubs",
				Setup: func(build api.PluginBuild) {
			// Stub Lit's own SSR packages and all their subpath imports
				// (e.g. @lit-labs/ssr/lib/dom-shim.js) -- we provide our own shim
				build.OnResolve(api.OnResolveOptions{Filter: `^@lit-labs/ssr(-dom-shim)?(/.*)?$`},
					...
					build.OnLoad(api.OnLoadOptions{Filter: ".*", Namespace: "ssr-stub"},
						func(args api.OnLoadArgs) (api.OnLoadResult, error) {
							// Re-export DOM shim globals that Lit's Node code paths need
							contents := `
