...
# ---------------------------------------------------------------------------
# Ruby/Rack + golit performance benchmark
#
...
IMAGE="golit-ruby"
CONTAINER_NAME="golit-ruby-bench"
PORT=9292
REQUESTS=100
BROWSER=false
TRACE=false
...
echo "============================================="
echo "  Ruby/Rack + golit Performance Benchmark"
echo "============================================="
...
cleanup
_start=$(now_ms)
podman run -d --name "$CONTAINER_NAME" -p "$PORT:9292" "$IMAGE" >/dev/null
wait_for_healthy
_end=$(now_ms)
...
_start=$(now_ms)
podman run -d --name "$CONTAINER_NAME" -p "$PORT:9292" \
